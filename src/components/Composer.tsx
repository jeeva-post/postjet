...
import { Send, Image as ImageIcon, CheckCircle2, Zap, Clock, Paperclip } from "lucide-react";
...
export default function Composer() {
  ...
  // 12 Original Platform Logos (React-friendly SVG)
  const platforms = [
    ...
    { name: "LinkedIn", color: "#0077B5", svg: <path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z" fill="currentColor"/> },
    { name: "Snapchat", color: "#FFFC00", svg: <path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm0 18.5c-3.5 0-6.25-1.75-6.25-4.25 0-1.25.75-2.25 2-2.75l-.25-.75c-1.5-.5-2.5-1.5-2.5-2.75 0-2 2.75-3.5 6-3.5s6 1.5 6 3.5c0 1.25-1 2.25-2.5 2.75l-.25.75c1.25.5 2 1.5 2 2.75 0 2.5-2.75 4.25-6.25 4.25z" fill="currentColor"/> },
    ...
    { name: "TikTok", color: "#000000", svg: <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.06-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.59-1.01V15.5c0 1.42-.3 2.81-1.02 4.05-1.48 2.4-4.5 3.55-7.2 2.89-2.1-.51-3.8-2.11-4.5-4.14-.8-2.28-.4-5.02 1.2-6.9 1.42-1.63 3.7-2.3 5.8-1.74v4.02c-1.12-.24-2.32.06-3.1.86-.78.8-1.02 2.02-.6 3.04.43 1.05 1.6 1.7 2.7 1.6 1.2-.1 2.2-1.1 2.4-2.3.06-.5.06-1 .06-1.5V.02z" fill="currentColor"/> },
    { name: "Threads", color: "#000000", svg: <path d="M12.01 22c-5.51 0-9.99-4.48-9.99-9.99s4.48-9.99 9.99-9.99 9.99 4.48 9.99 9.99-4.48 9.99-9.99 9.99zm0-18c-4.41 0-8 3.59-8 8s3.59 8 8 8 8-3.59 8-8-3.59-8-8-8zm4.5 12c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="currentColor"/> },
    { name: "Discord", color: "#5865F2", svg: <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037 19.736 19.736 0 0 0-4.885 1.515.069.069 0 0 0-.032.027C.533 9.048-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128c.126-.094.252-.192.372-.291a.074.074 0 0 1 .077-.01c3.927 1.793 8.18 1.793 12.061 0a.074.074 0 0 1 .077.01c.12.098.246.197.373.291a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.893.076.076 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.419-2.157 2.419zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.419-2.157 2.419z" fill="currentColor"/> }
  ];
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
      ...
      <div className="xl:col-span-8">
        <form action={postToTelegram} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl">
          <div className="flex items-center gap-3 mb-10 border-b pb-8">
            ...
            <h1 className="font-black text-sm uppercase tracking-widest text-slate-900">Broadcast Studio</h1>
          ...
          <textarea 
            ...
            placeholder="Type your story, Jeevan..."
          ...
          {/* Social Icons Distribution Grid */}
          <div className="mb-12">
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mb-8 ml-2">Channel Distribution</p>
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-6 xl:grid-cols-6 gap-8">
              {platforms.map((p, i) => (
                <div key={p.name} className="flex flex-col items-center gap-3 group cursor-pointer">
                  <div 
                    className={`w-16 h-16 rounded-[1.8rem] flex items-center justify-center transition-all duration-500 shadow-sm group-hover:scale-125 group-hover:-translate-y-4 group-hover:shadow-2xl ${i === 0 ? 'ring-4 ring-blue-500 ring-offset-4' : ''}`}
                    style={{ backgroundColor: `${p.color}15`, color: p.color }}
                  >
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none">{p.svg}</svg>
                  </div>
                  <span className="text-[8px] font-black uppercase text-slate-400 group-hover:text-slate-900 transition-colors tracking-tighter">{p.name}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="xl:col-span-4 sticky top-10 h-max">
        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-8 ml-6">Live Global Preview</p>
        <div className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl overflow-hidden max-w-[420px] mx-auto border-t-8 border-t-blue-600">
            <div className="p-8 flex items-center gap-4 bg-slate-50/50">
                <div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-2xl italic shadow-lg shadow-slate-200">PJ</div>
                <div>
                    <p className="text-sm font-black text-slate-900 italic uppercase">PostJet Pro</p>
                    <div className="flex items-center gap-1.5 mt-1">
                        <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Active Broadcasting</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-8 min-h-[400px]">
                ...
                <p className="text-slate-800 font-bold leading-relaxed whitespace-pre-wrap text-xl">
                    {text || "Start typing in the studio to see your viral post preview here..."}
                </p>
            </div>

            <div className="p-8 bg-blue-600 flex justify-between items-center text-white shadow-inner">
                <p className="text-[10px] font-black uppercase tracking-widest">Cross-Platform Sync Ready</p>
