...
export default function AccountsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white p-12">
      <header className="mb-12"><h2 className="text-5xl font-black italic uppercase italic">Connections</h2></header>
          ...
          return (
            <div key={p.id} className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] flex items-center justify-between group hover:border-blue-500/50 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div><h4 className="text-sm font-black uppercase">{p.id}</h4><span className={`text-[8px] font-black uppercase ${isConnected ? 'text-green-500' : 'text-slate-500'}`}>{isConnected ? 'Connected' : 'Ready'}</span></div>
              </div>
              <button onClick={() => window.location.href = `/api/auth/social/${p.key}`} className={`p-3 rounded-xl transition-all ${isConnected ? 'bg-slate-800 text-slate-400' : 'bg-blue-600 text-white'}`}>
