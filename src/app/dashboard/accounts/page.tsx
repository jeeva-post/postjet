...
export default function AccountsPage() {
  ...
  const handleConnect = (p: any) => {
    const callbackUrl = `${window.location.origin}/api/auth/callback/${p.key}`;
    let authUrl = "";
    ...
    if (p.key === "facebook" || p.key === "instagram") {
      ...
      // మిగిలిన యాప్స్ కి కూడా అఫీషియల్ లాగిన్ లింక్స్ ఇక్కడే యాడ్ అవుతాయి
      alert(`Redirecting to Secure ${p.id} Authorization...`);
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white flex">
      <aside className="w-72 bg-slate-900/50 border-r border-slate-800 p-8 h-screen sticky top-0">
        ...
        <nav className="space-y-4 font-black text-[11px] uppercase tracking-widest">
          <a href="/dashboard" className="text-slate-400 p-4 block hover:bg-slate-800 rounded-2xl">Dashboard</a>
          <a href="/dashboard/accounts" className="bg-blue-600 text-white p-4 block rounded-2xl">Connections</a>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-12 overflow-y-auto">
        <header className="mb-12">
          ...
          <p className="text-slate-400 mt-2">Connect accounts directly with a single click. No tokens needed.</p>
        </header>
              ...
              <div key={p.id} className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] flex items-center justify-between group hover:border-blue-500/50 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className={`text-[8px] font-black uppercase ${isConnected ? 'text-green-500' : 'text-slate-500'}`}>{isConnected ? 'Connected' : 'Offline'}</span>
                  </div>
                </div>
                <button onClick={() => handleConnect(p)} className={`p-3 rounded-xl transition-all ${isConnected ? 'bg-slate-800 text-slate-400' : 'bg-blue-600 text-white hover:scale-110'}`}>
