...
export default function AccountsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white p-12">
      <header className="mb-12"><h2 className="text-5xl font-black italic uppercase tracking-tighter">Mission Control</h2></header>
        ...
        {PLATFORMS.map((p) => {
          ...
          return (
            <div key={p.id} className="bg-slate-900/40 border border-slate-800 p-8 rounded-[2.5rem] flex items-center justify-between group hover:border-blue-500/50 transition-all">
              ...
              <button onClick={() => window.location.href = `/api/auth/social/${p.key}`} className="p-3 rounded-xl bg-blue-600 text-white shadow-lg hover:scale-110 transition-all">
