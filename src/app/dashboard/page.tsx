...
export default function PostJetUltimateDashboard() {
  ...
  const handlePublish = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      toast.success('Campaign launched successfully on all 12 platforms!');
      setLoading(false);
    }, 2500);
  };
      ...
      <aside className="w-64 bg-[#0F172A] text-white p-6 flex flex-col sticky top-0 h-screen hidden lg:flex">
        <div className="flex items-center gap-3 px-2 mb-10">
          <div className="bg-blue-600 p-2 rounded-xl shadow-lg">
            <Send size={22} className="-rotate-12 text-white" />
          </div>
          <span className="text-xl font-black tracking-tighter uppercase italic">PostJet</span>
        </div>
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="px-3 py-1.5 bg-green-50 text-green-600 rounded-lg text-[10px] font-bold border border-green-100 flex items-center gap-2">
                <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" /> CLOUD SYNC ACTIVE
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200/60">
              ...
              <div className="mt-4 flex gap-4">
                <button className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-xs font-bold text-slate-600 transition-all">
                  <ImageIcon size={16} /> Add Media
                </button>
              </div>
            </div>

            {/* 02. Channels Section (Exactly 2 Lines) */}
            <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200/60">
               <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[9px] font-bold text-slate-400">12 PLATFORMS SELECTED</span>
               </div>

               {/* lg:grid-cols-6 ensures 6 columns on large screens -> 2 lines for 12 items */}
