...
export default function TiktokIntegrationSettings() {
  ...
  const toggleConnection = async () => {
    ...
    setFeedback(`${action.charAt(0).toUpperCase() + action.slice(1)}ing tiktok...`);
    try {
  ...
  return (
    <div className="p-8 bg-white rounded-[2rem] shadow-2xl border border-slate-200">
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-black capitalize">tiktok</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-8">
        <div className="rounded-3xl border border-slate-100 p-5 bg-slate-50">
          ...
          <p className="text-slate-500 text-sm mt-2">Update publish profiles and sync credentials for tiktok content delivery.</p>
        </div>
      ...
      <div className="rounded-[2rem] bg-gradient-to-r from-blue-600 to-cyan-500 p-6 text-white shadow-xl mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-base leading-6">Enable auto-post routing and schedule content workflows to the tiktok channel.</p>
          </div>
        </div>
      </div>
